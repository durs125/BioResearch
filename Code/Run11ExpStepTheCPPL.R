...
yy<-1
 ...
 #  initialization <- c(.1,1,15 , 2.1,.2,2.1)
  #ideal initialization <- c(.01,.2,10.1,.5,.5,8.1 )
       ...
       startVar<-.01+.5*yy
      # startzz <- .01+.25*xx
  initialization <- c(startVar,2.5,startVar , .05 ,.1,.8) #test run
  # double startVar,  double  stepsize,   double maxVar,  double startzz,  double addzz, const double maxzz  )
 ...
 closeAllConnections()
 if (length(dir("/scratch/Infor"))) {
source("/scratch/Infor/R14GamaCode.R")
 }else{
 #setwd("/home/david/BioResearch/plotting")
 source("/home/david/BioResearch/plotting/R14GamaCode.R")
}
...
source("/home/david/BioResearch/Run/summeryStats.R")
