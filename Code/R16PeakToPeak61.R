...
outerLoop2 <- function(fiille) {
  ...
  avgProtein[1:floor(avgs/2)] <- datas$Protein[1:floor(avgs/2)]
...
Plots <- function(output2){
 ...
 if(TRUE){###LogPlots}
  ...
  pdf(paste("LogScalDelayedOscilationGamma,", runsN, "runs.pdf", sep = ""))
...
ifelse((length(dir("/scratch/Infor"))) ,
   ...
   fileN<-paste("stats1",output6[[1]][1],sep="")
write.csv(output7,file = fileN,row.names = rnam   )
rotate<-read.csv(fileN)
write.csv(file = paste("stats2",output6[[1]][1],sep=""),t(rotate))
